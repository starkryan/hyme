...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`Server-side fetching products for country: ${country} with operator: ${operator}`);
    ...
    console.log('Server-side products API response received for operator');
    ...
    console.error('Server-side error fetching products by operator:', {
    ...
    if (error.response) {
      if (error.response.status === 404) {
        return res.status(404).json({ 
          error: `No products available for ${country} with operator ${operator}.` 
        });
      }
      if (error.response.status === 400) {
        return res.status(400).json({ 
          error: 'Invalid country code or operator. Please try different values.' 
        });
      }
    }
